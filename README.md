...
your files stay in the tab and your rules in your browser's storage. PDF
statements from the supported banks are converted in the tab too, by the same
extractors behind the same reconciliation gate.
...
a figure the bank itself asserts to compare against. The browser page does the
same conversion for the four layouts it knows (CIBC, RBC, Scotiabank Ultimate,
ScotiaLine): pdf.js reads the PDF in the tab, `pdf_layout.py` rebuilds the
column-aligned text that poppler would have printed, and the same extractor
and the same gate decide. A statement that reconciles joins the run as a CSV
you can download and keep; one that does not is named with both figures and
not used.
...
into its virtual filesystem; `budget.run()` returns the report. A dropped PDF
goes to the same worker, where a vendored, pinned pdf.js supplies each page's
text fragments with their positions and `pdf_layout.py` and `pdf_import.py`
turn them into a reconciled CSV or a refusal. Nothing is fetched after the
runtime loads except pdf.js itself, on the first PDF, and every asset is served
from the page's own origin — the build vendors both rather than pointing at a
CDN. The reconstruction is tested on invented statements in
`tests/fixtures/pdf/`, one per bank, generated by `make_fixtures.py` there.

```bash
pixi run web-build      # assemble web/dist/ (about 16 MB; downloads Pyodide and pdf.js once)
